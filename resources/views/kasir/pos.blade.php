...
<div class="flex h-full" x-data="posApp({{ json_encode(['products' => $products, 'promos' => $promos, 'customers' => $customers]) }})">
    ...
    <div class="flex-1 overflow-y-auto p-4 bg-slate-50 dark:bg-slate-900">
        ...
        <div class="relative mb-3">
            <svg class="absolute left-3 top-2.5 w-4 h-4 text-slate-400 pointer-events-none" fill="none" stroke="currentColor" viewBox="0 0 24 24"><circle cx="11" cy="11" r="8" stroke-width="2"/><path d="m21 21-4.35-4.35" stroke-width="2"/></svg>
        ...
        <div class="flex items-center gap-2 bg-amber-50 border border-amber-300 rounded-xl px-3 py-2 mb-3 text-xs text-amber-800 font-medium flex-wrap">
            <svg class="w-3.5 h-3.5 shrink-0 text-amber-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z"/></svg>
                ...
                <div @click="!product.out && addToCart(product)"
                    ...
                    <template x-if="product.stock_mode === 'tracked'">
                        <div class="mt-1.5">
                            ...
                            <p class="text-[10px] mt-0.5 font-semibold" :class="product.out ? 'text-red-500' : product.stock <= 5 ? 'text-amber-500' : 'text-slate-400'"
    ...
    <div class="w-80 hidden lg:flex flex-col bg-white dark:bg-slate-800 border-l border-slate-100 dark:border-slate-700">
        ...
        <div class="p-4 border-b border-slate-100 dark:border-slate-700 flex items-center justify-between">
            <div class="flex items-center gap-2">
                <svg class="w-4 h-4 text-slate-600 dark:text-slate-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><circle cx="9" cy="21" r="1"/><circle cx="20" cy="21" r="1"/><path d="M1 1h4l2.68 13.39a2 2 0 002 1.61h9.72a2 2 0 002-1.61L23 6H6" stroke-width="2"/></svg>
        ...
        <div class="flex-1 overflow-y-auto p-3 space-y-2 min-h-0">
            <template x-if="cart.length === 0">
                <div class="flex flex-col items-center justify-center py-12 text-slate-400">
                    <svg class="w-10 h-10 mb-2 opacity-50" fill="none" stroke="currentColor" viewBox="0 0 24 24"><circle cx="9" cy="21" r="1"/><circle cx="20" cy="21" r="1"/><path d="M1 1h4l2.68 13.39a2 2 0 002 1.61h9.72a2 2 0 002-1.61L23 6H6" stroke-width="2"/></svg>
            ...
            <template x-for="item in cart" :key="item.id">
                <div class="flex items-center gap-2.5 p-2.5 bg-slate-50 dark:bg-slate-700 border border-slate-100 dark:border-slate-600 rounded-xl">
                    ...
                    <div class="flex items-center gap-1 shrink-0">
                        <button @click="changeQty(item.id, -1)" class="w-6 h-6 flex items-center justify-center rounded-lg border border-slate-200 dark:border-slate-500 text-slate-600 dark:text-slate-300 hover:bg-brand hover:text-white hover:border-brand transition-all text-sm font-bold">−</button>
        ...
        <div class="p-4 border-t border-slate-100 dark:border-slate-700 space-y-2">
            ...
            {{-- Customer --}}
            <div class="relative">
                <svg class="absolute left-3 top-2.5 w-3.5 h-3.5 text-slate-400 pointer-events-none" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2" stroke-width="2"/><circle cx="12" cy="7" r="4" stroke-width="2"/></svg>
                ...
                <div x-show="custDropOpen && custSuggestions.length > 0"
                    ...
                    <template x-for="c in custSuggestions" :key="c.id">
                        <div @mousedown.prevent="selectCustomer(c)" class="flex items-center gap-2.5 px-3 py-2.5 hover:bg-slate-50 dark:hover:bg-slate-600 cursor-pointer border-b border-slate-100 dark:border-slate-600 last:border-0">
            ...
            <div class="relative">
                <svg class="absolute left-3 top-2.5 w-3.5 h-3.5 text-slate-400 pointer-events-none" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07A19.5 19.5 0 014.69 12 19.79 19.79 0 011.61 3.38 2 2 0 013.6 1.18h3a2 2 0 012 1.72c.127.96.361 1.903.7 2.81a2 2 0 01-.45 2.11L7.91 8.96a16 16 0 006.13 6.13l1.41-1.41a2 2 0 012.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0122 16.92z" stroke-width="2"/></svg>
            ...
            <button @click="openPayment()"
                ...
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><rect x="1" y="4" width="22" height="16" rx="2" stroke-width="2"/><line x1="1" y1="10" x2="23" y2="10" stroke-width="2"/></svg>
                Bayar
            </button>
        </div>
    </div>
    ...
    <button x-show="cart.length > 0"
        ...
        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><circle cx="9" cy="21" r="1"/><circle cx="20" cy="21" r="1"/><path d="M1 1h4l2.68 13.39a2 2 0 002 1.61h9.72a2 2 0 002-1.61L23 6H6" stroke-width="2"/></svg>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-2xl w-full max-w-md max-h-[90vh] overflow-y-auto shadow-2xl" @click.stop>
            <div class="p-5">
                <h3 class="text-base font-extrabold mb-4 flex items-center gap-2">
                    <svg class="w-5 h-5 text-brand" fill="none" stroke="currentColor" viewBox="0 0 24 24"><rect x="1" y="4" width="22" height="16" rx="2" stroke-width="2"/><line x1="1" y1="10" x2="23" y2="10" stroke-width="2"/></svg>
                ...
                <div class="grid grid-cols-2 gap-2.5 mb-4">
                    <button @click="payMethod = 'cash'"
                            ...
                            class="border-2 rounded-xl p-3 flex flex-col items-center gap-1.5 transition-all">
                        <svg class="w-6 h-6" :class="payMethod === 'cash' ? 'text-brand' : 'text-slate-400'" fill="none" stroke="currentColor" viewBox="0 0 24 24"><rect x="2" y="6" width="20" height="12" rx="2" stroke-width="1.75"/><circle cx="12" cy="12" r="2" stroke-width="1.75"/><path d="M6 12h.01M18 12h.01" stroke-width="1.75"/></svg>
                        <p class="text-sm font-bold" :class="payMethod === 'cash' ? 'text-brand' : 'text-slate-600 dark:text-slate-300'">Tunai</p>
                    </button>
                    <button @click="payMethod = 'transfer'"
                            :class="payMethod === 'transfer' ? 'border-brand bg-brand/5' : 'border-slate-200 dark:border-slate-600'"
                            class="border-2 rounded-xl p-3 flex flex-col items-center gap-1.5 transition-all">
                        <svg class="w-6 h-6" :class="payMethod === 'transfer' ? 'text-brand' : 'text-slate-400'" fill="none" stroke="currentColor" viewBox="0 0 24 24"><rect x="3" y="3" width="18" height="18" rx="2" stroke-width="1.75"/><path d="M3 9h18" stroke-width="1.75"/><path d="M9 21V9" stroke-width="1.75"/></svg>
                        <p class="text-sm font-bold" :class="payMethod === 'transfer' ? 'text-brand' : 'text-slate-600 dark:text-slate-300'">Transfer</p>
                    </button>
                </div>
                ...
                {{-- Transfer detail --}}
                <div x-show="payMethod === 'transfer'" class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-700 rounded-xl p-3 mb-4">
                    <div class="flex items-center gap-2.5 mb-2">
                        <div class="w-9 h-9 rounded-lg bg-white flex items-center justify-center text-xs font-extrabold text-brand shadow-sm">BCA</div>
                        <div>
                            <p class="text-sm font-bold text-blue-800 dark:text-blue-300">Bank BCA</p>
                            <p class="font-extrabold text-base text-blue-800 dark:text-blue-200 tracking-widest">1234 5678 90</p>
                            <p class="text-xs text-blue-600">a.n. {{ auth()->user()->business?->name }}</p>
                        </div>
                        <button onclick="navigator.clipboard.writeText('1234567890')" class="ml-auto bg-brand text-white text-xs font-bold px-2.5 py-1.5 rounded-lg">Salin</button>
                    </div>
                    <p class="text-xs text-blue-600 text-center">Pastikan transfer sudah masuk sebelum konfirmasi</p>
                </div>
 
                <div class="flex gap-2">
                    <button @click="paymentModal = false" class="flex-1 py-2.5 border border-slate-200 dark:border-slate-600 rounded-xl text-sm font-semibold text-slate-600 dark:text-slate-300 hover:border-slate-300">Batal</button>
                    <button @click="submitCheckout()" :disabled="processing"
                            class="flex-2 flex-grow py-2.5 bg-brand text-white rounded-xl text-sm font-extrabold disabled:opacity-60 hover:bg-brand-dark transition-all">
            ...
            <div class="p-5">
                <div class="text-center mb-3">
                    <div class="inline-flex p-3 bg-emerald-100 dark:bg-emerald-900/30 rounded-full mb-2">
                        <svg class="w-7 h-7 text-emerald-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0"/></svg>
                    </div>
                ...
                <div x-show="lastTx?.customer?.phone" class="bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-700 rounded-xl p-3 mb-3">
                    ...
                    <button @click="sendWA()" class="w-full py-2 bg-[#25D366] text-white rounded-lg text-xs font-bold flex items-center justify-center gap-1.5">
                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M21 11.5a8.38 8.38 0 01-.9 3.8 8.5 8.5 0 01-7.6 4.7 8.38 8.38 0 01-3.8-.9L3 21l1.9-5.7a8.38 8.38 0 01-.9-3.8 8.5 8.5 0 014.7-7.6 8.38 8.38 0 013.8-.9h.5a8.48 8.48 0 018 8v.5z" stroke-width="2"/></svg>
                        Kirim Nota WA
                    </button>
                </div>
...
function posApp(data) {
    return {
        ...
        async searchCustomers() {
            if (this.custName.length < 1) { this.custSuggestions = []; this.custDropOpen = false; return; }
        ...
        async submitCheckout() {
            if (this.cashReceived !== '' && Number(this.cashReceived) < this.total && this.payMethod === 'cash') {
